...
**Published**: 2025-11-20 14:13:38

**PDF URL**: [https://arxiv.org/pdf/2511.16395v1](https://arxiv.org/pdf/2511.16395v1)
...
designs. To address this issue, we propose the frameworkCorrec-
tHDLthat leverages high-level synthesis (HLS) results as functional
...
ifies the target circuit’s functionality. The program is provided to an
...
nificant potential in enhancing the efficiency of HDL design [ 7–21].
...
specifications, whileCorrectBench[ 32,33] employs LLMs to build a
hybrid evaluation platform with a self-correction mechanism. The
frontier of LLM-assisted hardware design is further advanced inHigh-Level Synthesis (HLS) [34–40], C2HLSC [34–36], HLS-Repair
...
•An agentic HDL design framework,CorrectHDL, is proposed
and released as open source. By leveraging HLS-generated
HDL as a functional reference,CorrectHDLimplements an
...
•To mitigate attention dilution in LLMs, complex C/C++ algo-
...
•To efficiently debug the LLM-generated top-level HDL de-
...
mental results also demonstrate that the proposedCorrectHDL
achieves an average area reduction of 24.83% and power re-
duction of 26.98% compared with conventional HLS designs
and approaches the quality of human-engineered circuits.arXiv:2511.16395v1  [cs.AI]  20 Nov 2025

•  Labor -Intensive •  High Development Cost      •  Time -ConsumingOptimized
HDL</> 
SpecManual 
HDL D esign
RTL 
...
(b) Reliable High -Level SynthesisHLS 
Tool(a) Early -Stage LLM -Aided HDL Design
LLM
RTL S imulator
Error Log
•  Limited ASIC Adoption      • Limited HDL Readability• Hallucinations  • Difficulty in Functional Debugging
</> 
C/C+ +Template 
HDL D esignC /RTL
VerificationError -Prone 
HDL D esignSyntax 
Debugging
</> 
Spec
...
butInefficient HDLOutput of LLM -Gen HDL
Output of HLS -Gen HDLFunctional 
Discrepancy 
Functional 
Enhancement
Functionally 
...
1: voidaes256 (aes256_context * ctx, intk[32], intbuf[16]){…
2:   for(i= 0;i < 32;++i) {
3:     ctx->enckey[i] = k[i];ctx->deckey[i] = k[i];}
4:   for(i= 8; --i;){ expandEncKey (ctx->deckey, &rcon); }
6:   addRoundKey_cpy (buf,ctx->enckey,ctx ->key);...... 161: }（a)C++ Code (161Lines)
1: moduleaes256(
2:inputclk,start,input[255:0]key_in,output…);… 
3: reg[7:0]buf[0:15],enckey[0:31],key[0:31];reg[7:0]rcon;…
4: always @(posedge clk or negedge rst_n) begin
5:if(!rst_n) begin busy<= 1'b0; i<=4'd0; rcon<=8'd1; end
6:else if(start && !busy) begin 
7:load_state (key_in,buf);load_key(block_in,enckey );
8:load_key (key_in,key); addRound_cpy (......386: endmodule（b)LLM-Generated HDL (386 L ines)
1: moduleaes256(
2:clk, rst, ctx_key_triosy_lz , ctx_enckey_triosy_lz , ctx_de…
3:buf_rsc_zout , buf_rsc_lzout , buf_rsc_zin , buf_triosy_lz …);…
4:  input [7:0] EncKey_read_rom_sbox_rom_map_1_cmp_data_out;…
5:  wire aes_expandEncKey_1_i_aes_expandEncKey_1_i_mux_nl;…
6:  assign cpkey_mux1h_159_nl= MUX1HOT ((xor_1_cmx_sva_1[…]),
7:  (addkey_1_io_read_buf_rsc_sdt_1_sva[…]), 
8:  (EncKey_read_rom_sbox_rom_map_1_cmp_data_out[…]),
9:  mix_mix_xor_7_cmx_7_5_sva_1, (buf_rsci_din[ …]),
10:(ctx_key_rsci_data_in_d_mx0w1[…])...... 1879:endmodule（c)HLS-Generated HDL (1879 L ines)ErrorFigure 3: Examples of HDL Generation via LLM and HLS.
The concept ofCorrectHDLis illustrated in Fig. 2. The circuit
...
The input to theCorrectHDLframework consists of circuit de-
...
the-art solutions [ 22–30]. This choice makes the framework directly
...
community [ 41,42]. Accordingly, the proposedCorrectHDLeffec-
tively bridges an important gap in LLM-based circuit design.
The output of theCorrectHDLframework is a circuit generated by
...
far inferior readability and PPA. The objective ofCorrectHDLis thus
to enhance the functional correctness of the LLM-generated HDL
design using the HLS-generated HDL design as a reference.

HDL Design 
Agent
Knowledge Base: HDL RuleTool API : HLS Tool 
RTL Simulation Tool       
Logic Synthesis Tool
Scripts
 ·Simulation Execution
·Results Management
·Differential Comparison …Agent 
Toolkit
Task: Generate a 
correct HDL design
Cprogram withspec
...
PPA Analysis
Output: Functionally Correct HDL Design   (2)Differential Verification
&Functional EnhancementInput
LLM HLS
...
Template 
Syntactically 
Correct HDLTo Differential 
Verification
HDL Rule 
Library
Figure 5: LLM-assisted HDL generation.
3 Agentic HDL Design with LLMs Leveraging
HLS as Reference
...
3.1.1 C/C++ Program Decomposition.Directly feeding an entire
...
RTL Simulation 
Tool Manual< / >  Modified Syntactically Correct HDL 
HDL Rules
Conventions
…
……
Verilog Usage
NomenclatureSimilarity Search
(d) Prompt: You are an expert in HDL, 
we have the information as follows…
< / >  LLM-Gen HDL (DUT) ①
②
③
 Retrieved Template 
Follow the guidance to generate an 
a syntactically correct HDL design…
Script(a) Syntax error : incorrect declaration …
Iterative Repair
(c)  Template:  # **Syntax Error: Illegal declaration after the statement.
moduleaes256(inputstart…);
function [7:0] xt;
beginxt= x<<1; reg[7:0] tmp;…
(e) 
(b)
...
# ** S yntax Error Logmoduleaes256(inputstart…);
function [7:0] xt;
...
3.1.2 LLM-Assisted HDL Design Generation.After decomposition,
...
LLM–Generated HDL (DUT) HLS–Generated HDL (Golden Reference )
DUT P ath Golden PathSame Test Stimulus
RTL S imulation Results (.csv) HLS S imulation Results (.csv)Dump Output Results to Files
Mismatched Result
...
Still Functionally Incorrect HDL< / >  C/C++ Program SubBytes()… ShiftRows()… MixColumns()… AddRoundKey()… 
Iterative
RepairFigure 7: Differential verification for LLM-generated HDL.
Prompt :Your task istodebug functional errors intheLLM-Generated HDL
(DUT), andprovide acorrected version thatmatches thefunctionality oftheHLS-
Generated Golden Reference ...
Thecodes areprovided below :…
Erroneous Result from LLM-Gen HDL: HLS-generated Golden reference :
...
# Resul t1: [26, -29, 26, -23, 26, -29, …, -23]
# Result 2: [ -22, 1, 9, -13, -8, 0,…, -26, -2] # Testcase  1: Golden  # Testcase 2: Golden
# Result 1 : [26, -30, 26, -24, …, 26, -24]
# Result 2:  [18, 31, 7, 9, 11, -1,…, 13, 17] 
LLM’s Output Response :(Text simplified forclarity andspace)
Check 1:The small value differences (e.g.,−29vs.−30)come from inconsistent
rounding ,HDL uses truncation toward zero, while golden ac_fixed uses floor…
Check 2:Thefirsttestcase wasnearly correct, butthesecond case wascompletely
wrong …Latch itinST_WAIT_LBL andusethestored value forlater calculations …
#include <ac_fixed.h>
typedef ac_fixed <16,6> FT;…
staticFT dotP(…){FT result= 0;
for(inti= 0; i< NumF; i++) 
result += param[ i] * feature[ i];…}
staticvoid CGradi(FTypegrad[NumF…){
for(inti= 0; i< NumF; i++) 
grad[i] = scale * feature[ i];}…
voidSgdLR_sw (DT data[ NumF* NumT]…){
for(intid = 0; id < NumT; id++){
FT dot=dotP(t, &d[NumF*tid]);…< / > 
C++ Code 
with Specfunction [15:0] sgd;...
...
ic<=6’d0;dot<=16'sd0;state<=DOT;…< / > 
LLM-Gen
HDL (DUT)
Figure 8: Functional enhancement in LLM-generated HDL.
[50]. The retrieval process can be formalized as follows:
𝐸=embed(error_log), 𝑅 𝑖=embed(rule_template𝑖),(1)
where𝐸and𝑅𝑖denote the embedding vectors of the error log and
the𝑖th rule template. Given an error log 𝐸and a template 𝑅, the rule
template with the highest similarity is selected as follows:
sim(𝐸,𝑅)=𝐸·𝑅
∥𝐸∥∥𝑅∥, 𝑅∗=arg max
𝑅𝑖∈Rsim(𝐸,𝑅 𝑖),(2)
whereRdenotes all templates in the library, 𝑅∗represents the tem-
...
effective golden reference for verifying LLM-generated design.TABLE 1: Typical Functional Discrepancies in LLM-Generated HDL
and the Corresponding Corrections Driven byCorrectHDL.
Task DUT O utput HLS Reference Functional Discrepancy Corresponding Correction
Edge [-3, 66, …, -250, 108] [-31, 664 , …, -2479, 1081] Numeric scaling bug
Markov [5, 22, 17, …, 9, 14, 14] [5, 22, 17, …, 9, 14, 15] Boundary write -back bug Commit delayed byCorrect shift scaling
one cycle
DMM [262144, 0, 4096, …] [262144 , 4096 , 133120 ,…]Reset or delay bug Decouple load and compute
2: [8e, a2, b7, ..., 60, 89] 2: [6b, c1, be, ..., ef, 63] assignment timing bugAES1: [xx, xx, xx, ..., xx, xx] 1: [8e, a2, b7, ..., 60, 89] Lag due to synchronous Align output register with 
...
BFS [1, 0, 3, 0, 2, 1, 0, 1, …] [1, 1, 4, 4, 6, 7, 7, 8…] State retention across callsMerge gemm Kmp bfsspam nwmd aes Viterbi sha edge 
Add per-call initializationmipsTask c v
6md 97 270
5Kmp 88 240
1Gemm 38 179
9Edge 155 388
...
fication–feedback–repair iteration is repeated until functional equiv-
...
TABLE 2: Comparison ofCorrectHDLwith Baselines in Simulation Pass Rate, and with HLS in Area and Power Efficiency.
BenchmarkFunctional Simulation Pass Rate (%)HDL 
Readability 
(Code Lines)LLMPower Consumption (mW ) Area Overhead (µ m² )→
(NL as HDL
[29]
Input)LLM→
HDL
...
Instantiation                IntegrationShiftRows ()… LLM-Gen Sub -HDL 2
SubBytes()… LLM-Gen Sub -HDL 1
MixColumns()… LLM-Gen Sub -HDL 3
AddRoundKey()… LLM-Gen Sub -HDL 4
Verification & 
Functional
Repair 
as Stage 2LLM-GenTopHDL (DUT)  
HLS-GenTopHDL (Golden)  
Functionally 
Correct 
LLM-Gen 
Top HDL
...
at the top level, as in Section 3.2, to detect integration errors.To efficiently locate integration errors, boundary signal instru-
...
ficiency of the HDL designs produced byCorrectHDLand compared
...
library. InCorrectHDL, the GPT-5 model was employed as the LLM
via OpenAI APIs [ 49]. To ensure reliability and robustness, each task
was repeated 𝑛times.𝑛= 16 was used in the experiments. For each
...
𝑚/𝑛, where𝑚represents the number of successfully generated HDL
designs and𝑛denotes the total number of HDL designs.
Table 2 compares the HDL designs generated byCorrectHDLwith
...
algorithmic behavior. With the C/C++ decomposition step inCorrec-
...
by integrating such submodules together by the LLM. Columns 6-7
    ...
    Direct LLM for C Decomposition     C Decomposition with Rules in CorrectHDL    
506070809010C Decomp. Pass Rate (%)0
 AESBFSEdgeMFDMMKMPMIPSN-BodyDSA k SHASpamMarovFigure 10:C/C++ decomposition pass rate.
    Direct LLM for Syntax Repair    Syntax Repair with RAG in CorrectHDL   
506070809010Syntax Pass Rate (%)0
 AESBFSEdgeMFDMMKMPMIPSN-BodyDSASHASpamMarkov
Figure 11:Syntax pass rate of the LLM-generated HDL.
 
    
Direct LLM for Func. Repair 
Step-Wise Reasoning     
(bProposed Use of HLS as Reference
)
    
010203040506070809010Functional Simulation Pass Rate (%)0
 AESBFSEdgeMFDMMKMPMIPSN-BodyDSASHASpamMarkov
Figure 12:Functional pass rate of the LLM-generated sub-HDL.
    
    Direct LLM for Functional Repair
Task Decomp. for Functional Repair     Proposed CorrectHDL ( Only Top Design Feedback)
   
(bProposed CorrectHDL (Submodule Feedback)
)
    Functional Simulation Pass Rate (%)80
70
60
50
40
...
ofCorrectHDL, which demonstrates significant improvements in
functional correctness of the LLM-generated HDL.
To compare the quality of HDL code generated byCorrectHDL
and the traditional HLS tool, columns 8–9 in Table 2 report readabil-
ity and maintainability of HDL code, measured by the number of
lines of HDL code. This comparison showsCorrectHDLconsistently
generates more readable HDL than HLS. Columns 10–15 further
compare area and power under identical frequency, showing that
CorrectHDLproduces HDL designs with significantly lower area and
power consumption than HLS-generated HDL designs.
To demonstrate the effectiveness of the rule-based C/C++ de-
composition inCorrectHDL, we compare the proposed decomposi-
...
compilation pass rate achieved byCorrectHDL(with RAG) against
directly using the LLM. As shown in Fig. 11,CorrectHDLimproves
...
of submodules usingCorrectHDL, using the LLM directly, and using
...
LLM usage, whileCorrectHDL, using HLS as a functional reference,
further improves the pass rate by an average of 28.05%.
To demonstrate the top-level functional simulation pass rate of
the HDL design withCorrectHDL, which incorporates submodule
feedback from the HLS-generated reference during the final HDL
 
 
HLS-Generated HDL Design 
  
(b LLM-Generated HDL Design
(CorrectHDL)
)
   
051015202530354Area Overhead  (x10³ µm²)0
 AESBFSEdgeMFDMMKMPMIPSN-BodyDSASHASpamMarkovManual HDL DesignFigure 14:Comparison of area overhead.
...
024681012141618
 AESBFSEdgeMFDMMKMPMIPSN-BodyDSASHASpamMarkov Manual HDL Design 
...
and submodule boundary instrumentation,CorrectHDLoutperforms
...
generated byCorrectHDL, we compare them with those generated
...
synthesized circuits generated byCorrectHDLachieve an average
...
byCorrectHDLapproach the quality of human-engineered circuits
in many cases, while for a few benchmarks,CorrectHDLproduces
even better designs than those written by human engineers.
5 Conclusion
In this paper, we have proposedCorrectHDL, an agentic LLM-assisted
...
Experimental results on 12 real-world benchmarks show thatCorrec-
tHDLsignificantly improves both syntax and functional pass rates
...
Wan, Yingyan Celine Lin, “GPT4AIGChip: Towards Next-Generation AI Acceler-
ator Design Automation via Large Language Models,” IEEE/ACM International
...
Agent: An Intelligent Self-Correcting Script Generator for OpenROAD,” IEEE Inter-
national Conference on LLM-Aided Design (ICLAD), 2025.
[14] Yuan Pu, Zhuolun He, Shutong Lin, Jiajun Qin, Xinyun Zhang, Hairuo Han,
Haisheng Zheng, Yuqi Jiang, Cheng Zhuo, Qi Sun, David Z. Pan, Bei Yu, “A Multi-
...
Sriram Krishnamoorthy, Jeff Zhang, “MACO: A Multi-Agent LLM-Based Hard-
ware/Software Co-Design Framework for CGRAs,” arXiv: 2509.13557, 2025.
...
Maniatakos, Ozgur Sinanoglu, Johann Knechtel, “RTL-breaker: Assessing the Secu-
rity of LLMs Against Backdoor Attacks on HDL Code Generation,” ACM Design,
...
Ramesh Karri, Siddharth Garg, “A Large Language Model for Verilog Code Genera-
tion,” ACM Transactions on Design Automation of Electronic Systems, 2024.
[24] Mingjie Liu, Nathaniel Pinckney, Brucek Khailany, Haoxing Ren, “VerilogEval:
Evaluating large language models for verilog code generation,” IEEE/ACM Interna-
...
mann, “Enhancing Verilog Code Generation from Large Language Models via
Self-Consistency,” International Symposium on Quality Electronic Design, 2025.[26] Zhuorui Zhao, Bing Li, Grace Li Zhang, Ulf Schlichtmann, “VFocus: Better Verilog
...
Ramesh Karri, Brendan Dolan-Gavitt, Siddharth Garg, “Benchmarking Large Lan-
...
mann, “Paradigm-Based Automatic HDL Code Generation Using LLMs,” ACM/IEEE
...
[33] Ruidi Qiu, Grace Li Zhang, Rolf Drechsler, Ulf Schlichtmann, Bing Li, “Correct-
...
[36] Luca Collini, Andrew Hennessee, Ramesh Karri, Siddharth Garg, “Can rea-
soning models reason about hardware? an agentic HLS perspective,” arXiv
...
[38] Chenwei Xiong, Cheng Liu, Huawei Li, Xiaowei Li, “HLSPilot: LLM-based High-
Level Synthesis,” ACM/IEEE International Conference on CAD (ICCAD), 2024.
[39] Kangwei Xu, Bing Li, Grace Li Zhang, Ulf Schlichtmann, “HLSTester: Efficient Test-
ing of Behavioral Discrepancies with LLMs for High-Level Synthesis,” IEEE/ACM
International Conference on Computer Aided Design (ICCAD), 2025.
[40] Stefan Abi-Karam, Cong Hao, “HLS-Eval: A Benchmark and Framework for Eval-
uating LLMs on High-Level Synthesis Design Tasks,” IEEE ICLAD, 2025.
...
[42] Wenpin Hou, Zhicheng Ji, “Comparing large language models and human pro-
grammers for generating programming code,” Advanced Science, 2025.
...
[47] K. Xu, et al., “Logic Design of Neural Networks for High-Throughput and Low-
Power Applications,” IEEE/ACM Asia and South Pacific Design Automation Con-
