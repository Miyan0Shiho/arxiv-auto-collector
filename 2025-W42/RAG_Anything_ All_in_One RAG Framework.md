...
**Published**: 2025-10-14 09:25:35

**PDF URL**: [http://arxiv.org/pdf/2510.12323v1](http://arxiv.org/pdf/2510.12323v1)
...
RAG-ANYTHING: ALL-IN-ONE RAG FRAMEWORK
RAG-ANYTHING: ALL-IN-ONERAG FRAMEWORK
...
1arXiv:2510.12323v1  [cs.AI]  14 Oct 2025
...
knowledge repositories are inherentlyheterogeneous and multimodal, containing rich combinations
...
2.2 UNIVERSALREPRESENTATION FORHETEROGENEOUSKNOWLEDGE
...
documents into text segments, RAG-Anything introducesMultimodal Knowledge Unification. This
...
kiDecompose− − − − − − − → {c j= (tj, xj)}ni
j=1,(1)
where each unit cjconsists of a modality type tj∈text,image,table,equation, . . . and its corre-
sponding raw content xj. The content xjrepresents the extracted information from the original
knowledge source, processed in a modality-aware manner to preserve semantic integrity.
3

RAG-ANYTHING: ALL-IN-ONE RAG FRAMEWORK
ParallelParserMultimodal Knowledge Unification
...
ImageInfo...EquationInfoTextInfo...TableInfoHierarchical Text ExtractionImageCaption& Metadata ExtractionLaTeX Equation RecognitionTable Structure & Content ParsingDual-Graph Construction for Multimodal Knowledge (each document) Structured Content List
TextEncoder
Multi-modal Processors
Entity&RelationExtraction
BeekeeperBeeObserveKnowledgeGraph
MergedNode
ChildNodeParentNode: Multi-modal InstanceVLM/LLMMerged...Textual Multi-modalInfoCross-ModalKnowledgeGraphText-BasedKnowledgeGraphKGover All DocumentsMergedTextVDBMulti-modal VDB
VDBoverAllDocuments
QueryCould you share insights on the experimental results and data tables?
Response
Based on the experimental data, theresults revealed...Semantic Similarity MatchingStructural Knowledge NavigationQueryHigh-/Low-level KeysExtraction
Hybrid RetrievedInfo...Multi-modalInfoProcessingTextInfoProcessing
LLMTextEncoderVector Database
...
2.2.1 DUAL-GRAPHCONSTRUCTION FORMULTIMODALKNOWLEDGE
...
system first builds across-modal knowledge graphthat faithfully grounds non-textual modalities
within their contextual environment. It then constructs atext-based knowledge graphusing es-
...
unit. The first is adetailed description dchunk
j optimized for cross-modal retrieval. The second is
anentity summary eentity
...
neighborhood Cj={c k| |k−j| ≤δ} , where δcontrols the contextual window size. This ensures
...
its descriptiondchunk
j to identify fine-grained entities and relations:
(Vj,Ej) =R(dchunk
j ),(2)
where VjandEjdenote the sets of intra-chunk entities and their relations, respectively. Each
atomic non-text unit is associated with a multimodal entity node vmm
jthat serves as an anchor for
4

RAG-ANYTHING: ALL-IN-ONE RAG FRAMEWORK
its intra-chunk entities through explicitbelongs_toedges:
˜V={vmm
j}j∪[
jVj,(3)
˜E=[
...
2.2.2 GRAPHFUSION ANDINDEXCREATION
...
a comprehensive embedding table Tthat encompasses all components generated during the indexing
...
each components∈entities,relations,chunks is mapped to its corresponding dense representation:
T=emb(s) :s∈ V ∪ E ∪c jj,(5)
where emb(·) denotes the embedding function tailored for each component type. Together, the
unified knowledge graph Gand the embedding table Tconstitute the complete retrieval index
...
2.3 CROSS-MODALHYBRIDRETRIEVAL
...
embedding equsing the same encoder employed during indexing, ensuring consistency between
5
...
search between the query embeddinge qand all components stored in embedding tableT.
...
candidates from both pathways are unified into a comprehensive candidate pool: C(q) =C stru(q)∪
...
dark blueand second-best inlight blue. Domain categories include Academia (Aca.), Finance
...
MethodDomains TypesOverall
Aca. Fin. Gov. Law. News Txt. Mm. Una.
GPT-4o-mini 40.3 46.9 60.3 59.2 61.0 61.0 43.8 49.651.2
LightRAG 53.8 56.2 59.5 61.8 65.7 85.059.7 46.8 58.4
MMGraphRAG 64.352.8 64.940.0 61.5 67.6 66.0 60.5 61.0
RAGAnything 61.4 67.0 61.5 60.2 66.3 85.0 76.346.0 63.4
Table 3: Accuracy (%) on MMLongBench across different domains and overall performance. Best re-
sults are highlighted indark blueand second-best inlight blue.. Domain categories include Research
...
MethodDomainsOverall
Res. Tut. Acad. Guid. Broch. Admin. Fin.
GPT-4o-mini 35.5 44.024.6 33.1 29.5 46.831.1 33.5
LightRAG 40.834.1 36.2 39.4 41.044.4 38.3 38.9
MMGraphRAG 40.836.5 35.7 35.8 28.2 46.9 38.537.7
RAGAnything 46.6 43.5 38.7 43.9 34.045.7 43.6 42.8
...
144 dpi. Finally, all query results are evaluated for accuracy byGPT-4o-mini.
...
MethodDomains TypesOverall
Aca. Fin. Gov. Law. News Txt. Mm. Una.
Chunk-only 55.8 61.5 60.1 60.764.0 81.6 66.2 43.5 60.0
w/o Reranker 60.9 63.558.8 60.2 68.6 81.7 74.7 45.4 62.4
...
3.3 ARCHITECTURALVALIDATION WITHABLATIONSTUDIES
...
theseintra-modalstructures is crucial for accurate reasoning. We analyze two representative cases
...
RAG-ANYTHING: ALL-IN-ONE RAG FRAMEWORK
Multimodel Document
Evidence figure in the documentQuestion: Which model's style space shows a clearer separation
...
retriever to focus on thestyle-spacepanel for comparing cluster separation patterns. The system
...
column-of,header-applies-to, andunit-of. This structure enables precise navigation. The retriever
...
TypeRes. Tut. Acad. Guid. Broch. Admin. Fin.
...
Bench benchmarks. •DocBenchencompasses medium- to long-length documents spanning various
...
single-page newspapers. •MMLongBenchdemonstrates a broader spectrum of document types and
...
interconnected nodes. These are linked by semantic relations such asbar-ofandlabel-applies-to.
...
Multimodel Document
Evidence table in the documentQuestion: Which model combination for the Evidence Inference
...
headers and data cells become nodes connected through explicitrow-ofandcolumn-ofrelationships.
...
A.3 CONTEXT-AWAREMULTIMODALPROMPTING
...
A.4 ACCURACYEVALUATIONPROMPTDESIGN
...
our comprehensive experimental framework were conducted usingGPT-4o-mini, ensuring consistent
and reliable assessment standards across diverse question categories and specialized domains.
A.5 CHALLENGES ANDFUTUREDIRECTIONS FORMULTI-MODALRAG
...
Multimodel Document
Evidence table in the documentQuestion: Which model's style space shows a clearer separation
between different styles according to Figure 2?
GPT-4o-mini🤔: 
